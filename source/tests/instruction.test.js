...
test('instruction screen behavior', async () => {
  await page.goto(
    'file://' +
      path.resolve(__dirname, '../instruction_screen/instruction.html')
  );

  const instruction = await page.evaluate(() => {
    const instructionsList = document.getElementById('instructionsList');
    return Array.from(
      instructionsList.getElementsByClassName('instruction')
  ...
  const instructionsAfterClick = await page.evaluate(() => {
    return Array.from(document.getElementsByClassName('instruction')).map(
      (el) => el.style.display
    );
  });
